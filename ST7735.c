...
// Last Modified: 12/3/2019: Drew Hardie & Ben Liu 
...
void ST7735_DrawBitmap(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h){
  ...
  if((y - h ) < 0){                  // image exceeds top of screen
    i =(h - y)*originalWidth;  // skip the last cut off rows
    h = y ;
  }
...
void ST7735_DrawBitmap2(int16_t x, int16_t y, const uint16_t *image, int16_t w, int16_t h, uint16_t shine){
	int16_t skipC = 0;                      // non-zero if columns need to be skipped due to clipping
  int16_t originalWidth = w;              // save this value; even if not all columns fit on the screen, the image is still this width in ROM
  int i = w*(h - 1);
  if((x >= _width) || ((y - h + 1) >= _height) || ((x + w) <= 0) || (y < 0)){
  ...
  for(y=0; y<h; y=y+1){
    for(x=0; x<w; x=x+1){
      if(image[i]<=0x00FF){                                 // send the top 8 bits
      writedata((uint8_t)(image[i] >> 8));
                                        // send the bottom 8 bits
      writedata((uint8_t)image[i]);
      i = i + 1;                        // go to the next pixel
    }else{
		writedata((uint8_t)((shine) >> 8));
                                        // send the bottom 8 bits
     writedata((uint8_t)(shine));
     i = i + 1;                        // go to the next pixel
		}
	}
    i = i + skipC;
    ...
    for (j = 8; j>0; j--) {
      if (line & 0x1) {
        if (size == 1) // default size
          ST7735_DrawPixel(x-i, y+j, 0);
        else {  // big size
          ST7735_FillRect(x-(i*size), y+(j*size), size, size, 0);
        }
      } else if (bgColor != 0) {
        if (size == 1) // default size
          ST7735_DrawPixel(x-i, y+j, 0xCFFA);
        else {  // big size
          ST7735_FillRect(x-i*size, y+j*size, size, size, 0xCFFA);
        }
      }
      line >>= 1;
    }
